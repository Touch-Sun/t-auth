...
public class CrudUtil {
    ...
    public static <T> T addPrepare(Class<T> entity, Object dto) {
        try {
            ...
            // 得到基础用户操作信息
            BaseEntity baseEntity = BaseEntity.newInstance(UserHolder.get(), false);
            // 先拷贝前端表单信息
    ...
    public static <T> T updatePrepare(Class<T> entity, Object dto) {
        try {
            ...
            T result = entity.newInstance();
            // 得到基础用户操作信息
            BaseEntity baseEntity = BaseEntity.newInstance(UserHolder.get(), true);
            // 先拷贝用户操作信息
            BeanUtils.copyProperties(baseEntity, result);
            // 在拷贝前端表单信息
            BeanUtils.copyProperties(dto, result);
    ...
    public static <T> T singlePrepare(Class<T> entity, boolean update) {
        try {
            ...
            T result = entity.newInstance();
            // 得到基础用户操作信息
            BaseEntity baseEntity = BaseEntity.newInstance(UserHolder.get(), update);
            // 拷贝用户操作信息
            BeanUtils.copyProperties(baseEntity, result);
            // 返回
            return result;
